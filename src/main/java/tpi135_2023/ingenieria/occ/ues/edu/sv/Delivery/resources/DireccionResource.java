 ...
 */
@Path("/direccion")
public class DireccionResource implements Serializable {
    ...
    public Response Sucursal(Direccion direccion, @Context UriInfo info) {

        if (direccion != null) {
            try {
                Territorio territorio=territoriobean.findById(1);
                direccion.setIdTerritorio(territorio);
                direccionbean.crear(direccion);
            ...
            return Response.status(400).header(WRONG_PARAMETER, JsonbBuilder.create().toJson(direccion)).build();
        }
